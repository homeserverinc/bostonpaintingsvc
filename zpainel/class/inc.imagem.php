...
// funções para redimensionamento
	...
	*/	
	 ...
	 * @return void
	*/
	private function calculaPosicaoCropUp()
	{
		// média altura/largura
		$hm	= $this->altura / $this->nova_altura;
		$wm	= $this->largura / $this->nova_largura;
	 ...
	 * @return void
	*/
	private function resizeCropUp()
	{
		// calcula posicionamento do crop
		$this->calculaPosicaoCropUp();
        ...
        /**
	 * Redimensiona imagem sem cropar, proporcionalmente,
	 * sem preencher espaço vazio com cor rgb especificada
	 * @param
	 * @return void
	*/
	private function resizeProp()
	{
                // Get the original geometry and calculate scales
                $width = $this->largura;
                $height = $this->altura;

                $xscale=$this->largura/$this->nova_largura;
                $yscale=$this->altura/$this->nova_altura;

                // Recalculate new size with default ratio
                if ($yscale>$xscale){
                        $this->nova_largura = round($this->largura * (1/$yscale));
                        $this->nova_altura = round($this->altura * (1/$yscale));
                }
                else {
                        $this->nova_largura = round($this->largura * (1/$xscale));
                        $this->nova_altura = round($this->altura * (1/$xscale));
                }
...
// gera imagem de saída
	 ...
	 * @return void
	*/	
	public function grava( $destino='', $qualidade = 90 )
	{
