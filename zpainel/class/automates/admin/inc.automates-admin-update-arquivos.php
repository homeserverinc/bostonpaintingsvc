...
if ($_GET['dados']) {
	...
	foreach($dados as $key => $value) {
		...
		if ($basico == 0) $dados[$key] = "'$value'";
	}
	$this->mAtualizaDados($dados,$_GET['codigo2']);
	header("location: ?action=updateArquivos&codigo=".$_GET['codigo']."&palavra=".$_GET['palavra']."&pagina=".$_GET['pagina']."&codigo2=".$_GET['codigo2']."&palavra2=".urlencode($_GET['palavra2'])."&pagina2=".$_GET['pagina2']."&ok=yes");
} else {
	include "../class/inc.template.php";
	
	$cTpl = new cTemplate("../class/automates/templates/tpl.interfaceUpdateArquivos.htm");
	$cTpl->mSet("palavraRedir",urlencode($_GET['palavra']));
	$cTpl->mSet("paginaRedir",$_GET['pagina']);
	$cTpl->mSet("codigoRedir",$_GET['codigo']);
	$cTpl->mSet("palavra2Redir",urlencode($_GET['palavra2']));
	$cTpl->mSet("pagina2Redir",$_GET['pagina2']);
	$cTpl->mSet("codigo2Redir",$_GET['codigo2']);
	...
	$valorCampoVerificar = $this->mRetornaDados($_GET['codigo2']);
	$fileJavascript = "";
	...
	for($i=1;$i<count($campos);$i++) {
		if ($campos[$i]["file"]) {
			...
			if ($valorCampoVerificar[0][$campos[$i]["field"]] == "") {
				...
				$fileJavascript.= "\t\t\t$('#file".$campos[$i]["field"]."').jqUploader({ allowedExt:'".$realExtensoesFile."', allowedExtDescr: '".$realExtensoesFile."', nmArquivo: '".$this->v_tabelaBase.date("_Y_m_d__H_i_s_")."c".$_GET['codigo']."_r".str_pad(rand(0,99),2,"0",STR_PAD_LEFT).$alfabeto[rand(0,count($alfabeto)-1)].str_pad(rand(0,99),2,"0",STR_PAD_LEFT).$alfabeto[rand(0,count($alfabeto)-1)].str_pad(rand(0,99),2,"0",STR_PAD_LEFT).$alfabeto[rand(0,count($alfabeto)-1)].str_pad(rand(0,99),2,"0",STR_PAD_LEFT).$alfabeto[rand(0,count($alfabeto)-1)]."', nmCaminho: '".$this->v_caminho_imagem."' });\n";
			} else {
	...
	for($i=1;$i<count($campos);$i++) {
		//valor do campo
		$valorCampo = $this->mRetornaDados($_GET['codigo2']);
		...
		if (substr($campos[$i]["type"],0,3) == "int") {
			...
			if ($campos[$i]["relationTable"]) {
				...
				if ($campos[$i]["relationCodigo"]) {
					...
					$cTpl->mSet("tabelaRelationCodigoCampo",$campos[$i]["relationCampo"]);
					$cTpl->mSet("tabelaRelationCodigoCamposMostrar",$campos[$i]["relationCodigo"]);
					$cTpl->mSet("tabelaRelationCodigoCampoMostrar",$valorCampoGuardado[0][$campos[$i]["relationCampo"]]);
